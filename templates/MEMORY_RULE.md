---
name: claudelab-experiment-folder
description: Claude Code/API self meta-experiments stay in a separate claudelab workbench. Prevents project pollution + accumulates learning fragments in one place. Defines token-estimate disciplines and proactive proposal rules.
metadata:
  type: feedback
---

When working in a real project (your trading code, your SaaS, your client work, etc.) and you encounter **meta-level curiosity about Claude itself** — new features, unknown tags, slash command behavior, hook experiments, agent comparisons, model differences — **don't experiment in the real project**. Move to `~/<your-workspace>/claudelab/` (a sibling workbench) and verify/archive there.
...
- **Entry**: `cd ~/<your-workspace>/claudelab/`. Create subfolders by topic (e.g., `claudelab/mcp-protocol-spec-check/`, `claudelab/hook-experiments/`).
- **Output**: short `README.md` + experiment scripts/logs. Valuable findings get **promoted to memory** (reference or feedback type).
- **Real-project influence**: changes that should affect the real project (e.g., new MCP registration pattern) go through claudelab first, then PR-style into the project. Never experiment in the project directly.
- **Exception**: when the **real-project use itself is the learning event** (e.g., dogfooding your own tool's setup flow), do it in the project. claudelab is for project-unrelated meta curiosity only.

## Market evidence archive (added 2026-XX-XX)

claudelab is also a **verified market discovery archive**, not just a meta-experiment lab.
...
- **Archive only, do not execute immediately**. Maintain your "no premature project launch" meta-rule — claudelab archive is future reference, NOT launch approval.
- **Location**: `claudelab/market-leads/<short-slug>/README.md`. Include: discovery date, source, criteria cross-check, payment evidence, why this should NOT be folded into your existing tracks.
- **Reject cases**: simple idea / "this would be cool" intuition / ChatGPT-replicable wrapper / legal-medical-financial gray zones / capital-game categories. Don't archive these — keeps claudelab clean.
- User-discovered markets get the same archive treatment.

## Proactive proposal pattern (added 2026-XX-XX)

The rule is **bidirectional**. Not only the user proposes — the agent (Claude) can proactively propose **claudelab experiments** when encountering meta-curiosity during real work (new tools, suspected spec changes, behavior discrepancies, etc.).
...
| Ambiguous ("what should we do?" / "/claudelab") | Propose + recommend + **wait for explicit selection** |
...
- `feedback_audit_inflation_pmf_gate.md` — per-session archive cap. claudelab separation preserves the real-project cap.
- `feedback_no_portfolio.md` (your own version) — agent-initiated new project launches are nearly always rejected. claudelab is not a "new project," it's a learning fragment workbench. Product-track rules (payment, legal) don't apply.
- `feedback_rule_integrity.md` (your own version) — don't bypass this rule with "this time is different." Want to do Claude-meta work in the real project? Move to claudelab first.
